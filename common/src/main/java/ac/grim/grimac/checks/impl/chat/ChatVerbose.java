...
public final class ChatVerbose {
    private ChatVerbose() {
    }

    public static void register(@NotNull VerboseRegistry registry, @NotNull CheckRegistry checks) {
        registerStructured(registry, checks, ChatB.class, ChatB.V,
                formatter(ChatB.V, (in, ctx, out) -> out.text(in.rstr())));
        registerStructured(registry, checks, ChatC.class, ChatC.V,
                MultiActionsVerbose.movingFormatter(ChatC.V));
    }
