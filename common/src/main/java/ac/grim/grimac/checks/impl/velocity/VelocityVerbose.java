...
public final class VelocityVerbose {
    private VelocityVerbose() {
    }

    public static void register(@NotNull VerboseRegistry registry, @NotNull CheckRegistry checks) {
        registerStructured(registry, checks, KnockbackHandler.class, KnockbackHandler.V,
                formatter(KnockbackHandler.V, VelocityVerbose::renderKnockback));
        registerStructured(registry, checks, ExplosionHandler.class, ExplosionHandler.V,
                formatter(ExplosionHandler.V, VelocityVerbose::renderExplosion));
    }

    private static void renderKnockback(
            @NotNull VerboseBuf in,
            @NotNull VerboseRenderContext ctx,
            @NotNull VerboseSink out) {
        boolean ignored = in.rbool();
        double offset = in.rf64();
        out.text(ignored ? "ignored knockback" : "o: " + formatOffset(offset));
    }

    private static void renderExplosion(
            @NotNull VerboseBuf in,
            @NotNull VerboseRenderContext ctx,
            @NotNull VerboseSink out) {
        boolean ignored = in.rbool();
        double offset = in.rf64();
        out.text(ignored ? "ignored explosion" : "o: " + formatOffset(offset));
    }

    private static @NotNull String formatOffset(double offset) {
