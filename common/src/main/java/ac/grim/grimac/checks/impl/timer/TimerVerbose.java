...
public final class TimerVerbose {
    private TimerVerbose() {
    }

    public static void register(@NotNull VerboseRegistry registry, @NotNull CheckRegistry checks) {
        registerStructured(registry, checks, NegativeTimer.class, NegativeTimer.V,
                formatter(NegativeTimer.V, (in, ctx, out) ->
                        out.text("-").text(Long.toString(in.rvl()))));
        registerStructured(registry, checks, TickTimer.class, TickTimer.V,
                formatter(TickTimer.V, (in, ctx, out) ->
                        out.text("type=").text(in.rbool() ? "end" : "flying")
                                .text(", packets=").num(in.rvi())));
    }
