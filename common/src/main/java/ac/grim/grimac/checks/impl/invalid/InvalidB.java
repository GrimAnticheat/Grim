...
@CheckData(name = "InvalidB", check = "Invalid", type = "B", description = "Invalid action for players", setback = 1, experimental = false)
public class InvalidB extends Check implements PacketCheck {
    private long lastUseItem = -1L;
    private long useItem = -1L;
    private double buffer;

    public InvalidB(GrimPlayer player) {
        super(player);
    }
        ...
        if (event.getPacketType().equals(PacketType.Play.Client.INTERACT_ENTITY)) {
            ...
            if (wrapper.getAction().equals(WrapperPlayClientInteractEntity.InteractAction.ATTACK)) {
                if (this.player.packetStateData.isSlowedByUsingItem()) {
                    ...
                    // vanilla is unable to do blocking when sending more than two interact_entity
                    // packets
