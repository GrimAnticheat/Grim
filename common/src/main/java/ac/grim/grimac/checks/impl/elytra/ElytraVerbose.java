...
public final class ElytraVerbose {
    private ElytraVerbose() {
    }

    public static void register(@NotNull VerboseRegistry registry, @NotNull CheckRegistry checks) {
        registerStructured(registry, checks, ElytraB.class, ElytraB.V,
                formatter(ElytraB.V, (in, ctx, out) -> out.text(in.rbool() ? "no release" : "no jump")));
    }
