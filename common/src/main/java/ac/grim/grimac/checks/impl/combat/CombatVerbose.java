...
public final class CombatVerbose {
    private CombatVerbose() {
    }

    public static void register(@NotNull VerboseRegistry registry, @NotNull CheckRegistry checks) {
        registerStructured(registry, checks, MultiInteractA.class, MultiInteractA.V,
                formatter(MultiInteractA.V, (in, ctx, out) ->
                        out.text("lastEntity=").num(in.rzz())
                                .text(", entity=").num(in.rzz())
                                .text(", lastSneaking=").bool(in.rbool())
                                .text(", sneaking=").bool(in.rbool())));
        registerStructured(registry, checks, MultiInteractB.class, MultiInteractB.V,
                formatter(MultiInteractB.V, CombatVerbose::renderMultiInteractB));
    }

    private static void renderMultiInteractB(
