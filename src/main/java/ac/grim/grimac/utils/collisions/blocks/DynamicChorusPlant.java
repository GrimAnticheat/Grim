...
public class DynamicChorusPlant implements CollisionFactory {
    ...
    public Set<BlockFace> getLegacyStates(GrimPlayer player, ClientVersion version, int x, int y, int z) {
        ...
        StateType downBlock = player.getCompensatedWorld().getStateTypeAt(x, y - 1, z);
        StateType upBlock = player.getCompensatedWorld().getStateTypeAt(x, y + 1, z);
        StateType northBlock = player.getCompensatedWorld().getStateTypeAt(x, y, z - 1);
        StateType eastBlock = player.getCompensatedWorld().getStateTypeAt(x + 1, y, z);
        StateType southBlock = player.getCompensatedWorld().getStateTypeAt(x, y, z + 1);
        StateType westBlock = player.getCompensatedWorld().getStateTypeAt(x - 1, y, z);
