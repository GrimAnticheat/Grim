...
public class AimProcessor extends Check implements RotationCheck {
    ...
    public void process(final RotationUpdate rotationUpdate) {
        ...
        final float deltaXRot = rotationUpdate.getDeltaXRotABS();
        ...
        final float deltaYRot = rotationUpdate.getDeltaYRotABS();
        ...
        if (this.xRotMode.size() > SIGNIFICANT_SAMPLES_THRESHOLD) {
            final Pair<Double, Integer> modeX = this.xRotMode.getMode();
        ...
        if (this.yRotMode.size() > SIGNIFICANT_SAMPLES_THRESHOLD) {
            final Pair<Double, Integer> modeY = this.yRotMode.getMode();
    ...
    // TODO: make these variables have good names
    public static double convertToSensitivity(final double var13) {
        final double var11 = var13 / 0.15F / 8.0D;
