...
public class PacketEntityReplication extends Check implements PacketCheck {
    ...
    public void onPacketSend(PacketSendEvent event) {
        ...
        if (event.getPacketType() == PacketType.Play.Server.ENTITY_EFFECT) {
            ...
            if (player.getClientVersion().isOlderThan(ClientVersion.V_1_9) && ViaVersionUtil.isAvailable() && type.getId(null) > 23) {
            ...
            if (player.getClientVersion().isOlderThan(ClientVersion.V_1_13) && ViaVersionUtil.isAvailable() && type.getId(null) == 30) {
                event.setCancelled(true);
                return;
            }
